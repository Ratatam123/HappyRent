...
@oauth_authorized.connect_via(google_blueprint)  ## signal communicating log-in status
...
def logout():
    ...
    try:
        ...
        logout_user()        
        del google_blueprint.token ## muss in jedem Fall da sein?? 
        return redirect(url_for('google.login'))
...
def new_type():
    ...
    if current_user.is_authenticated and type_form.validate_on_submit():
        ...
        db.session.commit()
        flash('Category "{}" has been saved, please add more details below.'.format(item_type.type.capitalize()), 'success') ## success => communicates with Bootstrap
...
def new_item():
    ...
    if current_user.is_authenticated and form.validate_on_submit():
        ...
        item_type = db.session.query(PropertyType).order_by(asc(PropertyType.date_posted)).first()
